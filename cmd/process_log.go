...
func processLockIndexer(client *ethclient.Client, processor adapters.LogIndexer[adapters.Lock], blockNumber int64) error {
	...
	senders, err := processor.Index(ctx, logs...)
	if err != nil {
		log.Fatalf("Failed to process the logs: %v", err)
		return err
	}
